...
export async function fileHasRubyShebang(path: string): Promise<boolean> {
  ...
  return handle.read(buffer, 0, buffer.length, 0).then(
    async (info) => {
      await handle.close();
      return /^#!.*\bruby\b/u.test(buffer.subarray(0, info.bytesRead).toString("utf8"));
    },
    async (error: unknown) => {
      await handle.close();
      throw error;
...
export async function rubyGemspecPaths(
  ...
  for (const entry of entries) {
    ...
    const nestedEntries = await readdir(join(root, entry.name), { withFileTypes: true }).then(
      (value) => value,
      () => [],
    );
