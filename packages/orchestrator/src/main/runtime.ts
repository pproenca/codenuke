...
const shellOk = async (cmd: string, cwd: string, env: NodeJS.ProcessEnv, timeout = COMMAND_TIMEOUT): Promise<boolean> =>
  (await runShellGroup(cmd, { cwd, env, timeout })).ok;
...
const writeState = (path: string, state: EngineState): void => writeFileSync(path, JSON.stringify(state, null, 2));

const resolveBaselineSha = (config: Config, env: NodeJS.ProcessEnv): string =>
  run("git", ["rev-parse", "--verify", "--end-of-options", `${config.baseline}^{commit}`], { cwd: config.repo, env }).trim();

function assertStateBaselineSha(config: Config, env: NodeJS.ProcessEnv, state: EngineState): void {
  try {
    const resolved = run("git", ["rev-parse", "--verify", "--end-of-options", `${state.baselineSha}^{commit}`], {
      cwd: config.repo,
      env,
    }).trim();
    if (resolved !== state.baselineSha) throw new Error("baselineSha resolved to a different commit");
  } catch (cause) {
    throw new Error("invalid autoloop state: baselineSha does not resolve to a repository commit", { cause });
  }
}
...
function artifactReadiness(status: ArtifactStatus): { present: boolean; stale: boolean; usable: boolean } {
  return { present: status.artifact != null, stale: status.stale, usable: status.usable };
}

function valueProxyReadiness(status: ValueProxyStatus): { present: boolean; stale: boolean; usable: boolean } {
  return { present: status.artifact != null, stale: false, usable: status.usable };
}

async function isolatedChecks(config: Config, env: NodeJS.ProcessEnv): Promise<{
  baselineExists: boolean;
  baselineGreen: boolean;
  typecheckOk: boolean;
}> {
  const worktree = `${config.worktree}-doctor-${slug(Date.now())}`;
  const resolved = tryRun("git", ["rev-parse", "--verify", "--end-of-options", config.baseline], { cwd: config.repo, env });
  if (!resolved.ok) {
...
export async function runDoctor(env: Env = process.env, cwd = process.cwd()): Promise<CommandResult> {
  ...
  return { exitCode: report.includes("ready") && !report.includes("not ready:") ? 0 : 2, stdout: lines(report) };
}

function targetL(config: Config, ref: string): number {
  const files = run("git", ["ls-tree", "-r", "-z", "--name-only", ref, "--", config.target], { cwd: config.worktree })
    .split("\0")
...
async function typeErrors(config: Config, env: NodeJS.ProcessEnv): Promise<number> {
  ...
  const result = await runShellGroup(config.typeCheckCommand, { cwd: config.worktree, env, timeout: COMMAND_TIMEOUT });
...
async function initializeWorktree(
  ...
  if (existsSync(config.state)) return;
...
function changedSource(config: Config): string[] {
  return run("git", ["diff", "-z", "--name-only", "HEAD", "--", config.srcDir], { cwd: config.worktree })
    .split("\0")
    .filter((value) => value.length > 0 && isAllowedReducePath(value, config.srcDir));
}
...
async function scoreCandidate(config: Config, env: NodeJS.ProcessEnv, state: EngineState): Promise<ScoreResult | null> {
  ...
  const fenceRegions = (fence?.regions ?? {}) as Record<string, { admissible?: boolean; p?: number } | undefined>;
...
function benchmarkInfo(config: Config): { benchmarkRel: string; benchmarkInsideRepo: boolean } {
  ...
  return {
    benchmarkRel,
    benchmarkInsideRepo: Boolean(benchmarkRel && !benchmarkRel.startsWith("..") && !benchmarkRel.startsWith("/")),
  };
}

function hideBenchmark(config: Config, benchmarkRel: string, benchmarkInsideRepo: boolean): void {
  if (benchmarkInsideRepo) rmSync(`${config.worktree}/${benchmarkRel}`, { recursive: true, force: true });
}

function restoreBenchmark(config: Config, benchmarkRel: string, benchmarkInsideRepo: boolean, env: NodeJS.ProcessEnv): void {
  if (benchmarkInsideRepo) {
    const restored = tryRun("git", ["restore", "--staged", "--worktree", "--", benchmarkRel], {
      cwd: config.worktree,
      env,
    });
    if (!restored.ok) tryRun("git", ["checkout", "HEAD", "--", benchmarkRel], { cwd: config.worktree, env });
  }
}
...
function dirtyPaths(config: Config, benchmarkRel: string, benchmarkInsideRepo: boolean): string[] {
  const fields = tryRun("git", ["status", "--porcelain=v1", "-z", "-uall"], { cwd: config.worktree }).out
    .split("\0")
    .filter(Boolean);
  const paths: string[] = [];
  for (let i = 0; i < fields.length; i += 1) {
    ...
    for (const candidate of [path, source].filter((value): value is string => Boolean(value))) {
      if (
        !isHiddenBenchmarkDeletion({ benchmarkInsideRepo, benchmarkRel, path: candidate, status }) &&
...
function dirtyPathsAfterProposer(config: Config, benchmarkRel: string, benchmarkInsideRepo: boolean): string[] {
  const paths = dirtyPaths(config, benchmarkRel, benchmarkInsideRepo);
  if (existsSync(`${config.worktree}/node_modules`)) paths.push("node_modules");
...
function compactProcessOutput(result: { readonly stdout?: string; readonly stderr?: string }): string {
...
function readReducerProgram(config: Config): string {
  if (!existsSync(config.program)) {
    throw new Error(`program.md missing at ${config.program}; set CN_PROGRAM or reinstall the packaged runtime data.`);
  }
  const program = readFileSync(config.program, "utf8");
  if (program.trim().length === 0) {
    throw new Error(`program.md is empty at ${config.program}; set CN_PROGRAM or reinstall the packaged runtime data.`);
  }
  return program;
}

async function runProposer(config: Config, env: NodeJS.ProcessEnv, prompt: string, regionKey: string): Promise<ProcessResult> {
  const proposerEnv = {
    ...env,
    CN_REGION: regionKey,
  ...
  if (env.CN_PROPOSER) {
    return runShellGroup(env.CN_PROPOSER, { cwd: config.worktree, timeout: config.proposerTimeoutMs, env: proposerEnv });
  }
...
function regionTarget(config: Config, regionKey: string): string {
  if (config.srcDir === ".") return regionKey === "." ? "." : `${regionKey}/`;
  if (regionKey === config.srcDir) return `${config.srcDir}/`;
...
export async function runAutoloop(
  ...
  try {
    ...
    if (existingState) assertStateBaselineSha(config, env as NodeJS.ProcessEnv, existingState);
  } catch (error) {
    return { exitCode: 1, stdout: `${error instanceof Error ? error.message : String(error)}\n` };
  }
  let startupBaselineSha: string;
  try {
    startupBaselineSha = existingState?.baselineSha ?? resolveBaselineSha(config, env as NodeJS.ProcessEnv);
  } catch {
    return { exitCode: 1, stdout: `baseline ${config.baseline} not found\n` };
  }
  const startupConfig = artifactConfig(config, existingState ?? { baselineSha: startupBaselineSha, baselineTsc: 0, startL: 0, accepted: [], iter: 0 });
  ...
  const candidates = fence
    ? inScopeRegions(
        { regions: (fence.regions ?? {}) as Record<string, { admissible?: boolean; lo?: number } | undefined> },
        config.regions,
        config.target,
        config.srcDir,
      )
  ...
  if (startup) return { exitCode: startup.exitCode, stdout: `${startup.message}\n` };
  ...
  if (!existsSync(config.results)) writeFileSync(config.results, `${formatResultsHeader()}\n`);
  out.push(
  ...
  for (let i = 1; i <= iterations; i += 1) {
    ...
    const scoped = inScopeRegions({ regions: regionMap }, config.regions, config.target, config.srcDir);
    ...
    out.push(
      `\n--- iter ${i}/${iterations} [${mode}] ${activeRegion} fence ${
        region ? `${((region.p ?? 0) * 100).toFixed(0)}% lo=${((region.lo ?? 0) * 100).toFixed(0)}%` : "unmeasured"
      } ---`,
    );
    if (mode === "raise") {
      ...
      if (specs.length === 0) {
        logRow(config, out, {
          ...
          dCx: 0,
          behavior: "-",
          mfence: region ? Number(region.p ?? 0).toFixed(2) : "-",
          loss: "-",
          status: "raise-skip",
      ...
      const loBefore = Number(region?.lo ?? 0);
      ...
      if (!proposal.ok) {
        ...
        restoreBenchmark(config, bench.benchmarkRel, bench.benchmarkInsideRepo, env as NodeJS.ProcessEnv);
        ...
        logRow(config, out, {
          ...
          dCx: 0,
          behavior: "-",
          mfence: Number(region?.p ?? 0).toFixed(2),
          loss: "-",
      ...
      const dirtyAfterRaise = dirtyPathsAfterProposer(config, bench.benchmarkRel, bench.benchmarkInsideRepo);
      ...
      const disallowed = dirtyAfterRaise.filter((path) => !isAllowedRaisePath(path, config.testLayout.roots));
      ...
      const replayFenceStatus = fenceArtifactStatus(artifactConfig(config, replayStateForArtifacts));
      const replayRegion = ((replayFenceStatus.artifact?.regions ?? {}) as Record<string, FenceRegionRuntime | undefined>)[activeRegion];
      if (!replayRegion) {
        discardTipCommit(config);
        logRow(config, out, {
          iter: i,
      ...
      raised += 1;
      const status = Number(replayRegion.lo ?? 0) > loBefore + 1e-9 ? "raise" : "raise-nogain";
      const keptCommit = status === "raise";
      if (!keptCommit) discardTipCommit(config);
      logRow(config, out, {
        iter: i,
        ...
        dCx: 0,
        behavior: "true",
        mfence: Number(replayRegion.p ?? 0).toFixed(2),
        loss: "-",
        status,
        description: `${activeRegion} fence ${(loBefore * 100).toFixed(0)}%→${(Number(replayRegion.p ?? 0) * 100).toFixed(0)}% lo=${(Number(replayRegion.lo ?? 0) * 100).toFixed(0)}%${replayRegion.admissible ? " ADMISSIBLE✓" : ""}`,
      });
      continue;
    }
    ...
    try {
      ...
      restoreBenchmark(config, bench.benchmarkRel, bench.benchmarkInsideRepo, env as NodeJS.ProcessEnv);
      restoreRuntimeDeps(config);
      return { exitCode: 1, stdout: lines([...out, error instanceof Error ? error.message : String(error)]) };
    }
    const proposal = await runProposer(
      config,
      env as NodeJS.ProcessEnv,
    ...
    const disallowed = dirtyPathsAfterProposer(config, bench.benchmarkRel, bench.benchmarkInsideRepo).filter(
      (path) => !isAllowedReducePath(path, config.srcDir),
    );
    if (disallowed.length > 0) {
      restoreBenchmark(config, bench.benchmarkRel, bench.benchmarkInsideRepo, env as NodeJS.ProcessEnv);
      restoreRuntimeDeps(config);
    ...
    if (score.keep) {
      ...
      logRow(config, out, {
        ...
        loss: score.loss?.toFixed(3) ?? "+Inf",
        status: "revert",
        description: `${desc} | ${score.gates.G1prime ? "" : "G1′ "}${score.gates.G1 ? "" : "G1 "}${score.gates.G3 ? "" : "G3 "}${score.gates.G4 ? "" : "G4↓"}`.trim(),
      });
    }
  }
  ...
  if (existsSync(config.state)) {
    const state = readState(config.state);
    const headSha = run("git", ["rev-parse", "--verify", "--end-of-options", "HEAD"], { cwd: config.worktree }).trim();
