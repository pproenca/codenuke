...
export function mannWhitney(a, b) {
  ...
  return { U1, auc: U1 / (n1 * n2), z, p: 2 * (1 - normalCDF(Math.abs(z))), rankBiserial: 2 * (U1 / (n1 * n2)) - 1, n1, n2 };
}
...
export function bootstrapAUC(scores, labels, B = 2000) {
  const n = scores.length, aucs = [];
  for (let b = 0; b < B; b++) {
    const s = [], l = [];
    for (let i = 0; i < n; i++) { const j = (Math.random() * n) | 0; s.push(scores[j]); l.push(labels[j]); }
...
export function bootstrapRatioMedian(a, b, B = 5000) {
  const med = (xs) => quantile([...xs].sort((x, y) => x - y), 0.5);
  const r = [];
  for (let i = 0; i < B; i++) {
    const aa = a.map(() => sample(a)), bb = b.map(() => sample(b));
    const mb = med(bb); if (mb !== 0) r.push(med(aa) / mb);
  }
  r.sort((x, y) => x - y);
...
export function bootstrapAUCDiff(scoresA, scoresB, labels, B = 2000) {
  const n = labels.length, diffs = [];
  for (let b = 0; b < B; b++) {
    const sa = [], sb = [], l = [];
    for (let i = 0; i < n; i++) { const j = (Math.random() * n) | 0; sa.push(scoresA[j]); sb.push(scoresB[j]); l.push(labels[j]); }
    if (l.includes(1) && l.includes(0)) diffs.push(aucFromScores(sa, l) - aucFromScores(sb, l));
  }
  diffs.sort((x, y) => x - y);
  return { diff: aucFromScores(scoresA, labels) - aucFromScores(scoresB, labels), lo: quantile(diffs, 0.025), hi: quantile(diffs, 0.975) };
}
