...
pnpm build
pnpm test
node orchestrator/dist/cli.js --version
```
...
- Root package: public package `codenuke@0.4.0`, owns the published `codenuke` CLI bin.
- `orchestrator/`, `stats/`, `value-proxy/`, `json-io/`, `guards/`, `measure/`, `exec/`, `config/`, `artifacts/`, `substrate/`, `changecost/`, `calibrate/`, `fence/`, `scorer/`: internal workspace packages bundled into the root tarball.
