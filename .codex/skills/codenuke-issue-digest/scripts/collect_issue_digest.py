#!/usr/bin/env python3
"""Collect recent pproenca/codenuke issue activity for owner-focused digests."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Collect recent GitHub issue activity for a codenuke owner digest."
    )
    parser.add_argument(
        "--repo", default="pproenca/codenuke", help="OWNER/REPO, default pproenca/codenuke"
    )
    parser.add_argument(
        "--labels",
        nargs="+",
        default=[],
        help="Feature-area labels owned by the digest recipient, for example: cli mapper",
    )
    parser.add_argument(
        "--all-labels",
        action="store_true",
