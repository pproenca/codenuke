...
def test_build_search_queries_uses_each_owner_and_kind_label():
    ...
    queries = collect_issue_digest.build_search_queries(
        "pproenca/codenuke", ["cli", "mapper"], since
    )

    assert queries == [
        "repo:pproenca/codenuke is:issue updated:>=2026-04-25 label:cli label:bug",
        "repo:pproenca/codenuke is:issue updated:>=2026-04-25 label:cli label:enhancement",
        "repo:pproenca/codenuke is:issue updated:>=2026-04-25 label:mapper label:bug",
        "repo:pproenca/codenuke is:issue updated:>=2026-04-25 label:mapper label:enhancement",
    ]
...
def test_build_search_queries_can_scan_all_labels():
    ...
    queries = collect_issue_digest.build_search_queries(
        "pproenca/codenuke", [], since, all_labels=True
    )

    assert queries == [
        "repo:pproenca/codenuke is:issue updated:>=2026-04-25 label:bug",
        "repo:pproenca/codenuke is:issue updated:>=2026-04-25 label:enhancement",
    ]
...
def test_summarize_issue_keeps_new_comments_and_reaction_signals():
    ...
    issue = {
        "number": 123,
        "title": "CLI does not redraw",
        "html_url": "https://github.com/pproenca/codenuke/issues/123",
        "state": "open",
    ...
    comments = [
        {
            ...
            "html_url": "https://github.com/pproenca/codenuke/issues/123#issuecomment-1",
            "user": {"login": "bob"},
        ...
        },
        {
            ...
            "html_url": "https://github.com/pproenca/codenuke/issues/123#issuecomment-2",
            "user": {"login": "carol"},
            "author_association": "NONE",
            "body": "Older comment.",
    ...
    assert summary == {
        "number": 123,
        "title": "CLI does not redraw",
        "description": "CLI does not redraw",
        "url": "https://github.com/pproenca/codenuke/issues/123",
        "state": "open",
        "author": "alice",
        "author_association": "NONE",
        "created_at": "2026-04-24T20:00:00Z",
        ...
        "new_comments": [
            {
                ...
                "created_at": "2026-04-25T11:00:00Z",
                "updated_at": "2026-04-25T11:00:00Z",
                "url": "https://github.com/pproenca/codenuke/issues/123#issuecomment-1",
...
def test_summarize_issue_filters_non_owner_or_non_kind_labels():
    ...
    issue_without_owner["labels"] = [{"name": "bug"}, {"name": "mapper"}]
...
def test_fetch_comments_uses_since_filter_and_page_cap(monkeypatch):
    ...
    payload = collect_issue_digest.fetch_comments(
        "pproenca/codenuke", 123, since=since, max_pages=1
    )
    ...
    assert calls == [
        [
            "api",
            "repos/pproenca/codenuke/issues/123/comments?since=2026-04-25T00%3A00%3A00Z&per_page=100&page=1",
        ]
    ]
...
def test_issue_description_prefers_title_over_body_noise():
    issue = {
        "title": "Codenuke CLI: provider child process is not cleaned up after review",
        "body": "A later crash mention should not override the title-level symptom.",
        "labels": [{"name": "mapper"}, {"name": "bug"}],
    }

    description = collect_issue_digest.issue_description(issue)
    assert "provider child process" in description
...
def test_attention_markers_count_human_user_interactions():
    ...
    issue = {
        ...
        "html_url": "https://github.com/pproenca/codenuke/issues/456",
        "state": "open",
        "created_at": "2026-04-25T01:00:00Z",
        "updated_at": "2026-04-25T12:00:00Z",
        "user": {"login": "alice"},
        "labels": [{"name": "bug"}, {"name": "provider"}],
    }
    comments = [
        {
            "id": idx,
...
def test_reactions_count_toward_attention_markers():
    ...
    issue = {
        ...
        "html_url": "https://github.com/pproenca/codenuke/issues/789",
        "state": "open",
...
def test_user_interactions_are_deduped_by_human_login():
    ...
    issue = {
        ...
        "html_url": "https://github.com/pproenca/codenuke/issues/790",
        "state": "open",
        "created_at": "2026-04-25T01:00:00Z",
        "updated_at": "2026-04-25T12:00:00Z",
        "user": {"login": "Alice"},
...
def test_digest_rows_are_table_ready_with_concise_descriptions():
        ...
        [
            {
                ...
                "url": "https://github.com/pproenca/codenuke/issues/1",
            ...
            },
            {
                ...
                "url": "https://github.com/pproenca/codenuke/issues/2",
                "owner_labels": ["provider"],
                "kind_labels": ["bug"],
                "state": "open",
                "attention": True,
                "attention_level": 1,
    ...
    assert rows[0] == {
        "ref": 1,
        "ref_markdown": "[1](https://github.com/pproenca/codenuke/issues/2)",
        "marker": "🔥",
        "attention_marker": "🔥",
        "number": 2,
        "description": "High-volume bug report",
...
def test_summary_inputs_are_model_ready_without_preclustering():
    issues = [
        {
            "number": 20,
            "title": "Mapper includes generated dist files",
            "description": "Mapper output or provider schema failure",
            "url": "https://github.com/pproenca/codenuke/issues/20",
            "labels": ["mapper", "bug"],
            "owner_labels": ["mapper"],
            "kind_labels": ["bug"],
            "attention": False,
        ...
        {
            "number": 21,
            "title": "Provider command output waits until timeout",
            "description": "Windows command timeout/capture problem",
            "url": "https://github.com/pproenca/codenuke/issues/21",
            "labels": ["mapper", "bug"],
            "owner_labels": ["mapper"],
            "kind_labels": ["bug"],
            "attention": False,
        ...
        {
            "number": 22,
            "title": "CLI report omits selected findings",
            "description": "Report filtering workflow failure",
            "url": "https://github.com/pproenca/codenuke/issues/22",
            "labels": ["mapper", "bug"],
            "owner_labels": ["mapper"],
            "kind_labels": ["bug"],
            "attention": False,
    ...
    assert rows == [
        {
            "ref": 1,
            "ref_markdown": "[1](https://github.com/pproenca/codenuke/issues/20)",
            "number": 20,
            "title": "Mapper includes generated dist files",
            "description": "Mapper output or provider schema failure",
            "url": "https://github.com/pproenca/codenuke/issues/20",
        ...
        {
            "ref": 2,
            "ref_markdown": "[2](https://github.com/pproenca/codenuke/issues/21)",
            "number": 21,
            "title": "Provider command output waits until timeout",
            "description": "Windows command timeout/capture problem",
            "url": "https://github.com/pproenca/codenuke/issues/21",
        ...
        {
            "ref": 3,
            "ref_markdown": "[3](https://github.com/pproenca/codenuke/issues/22)",
            "number": 22,
            "title": "CLI report omits selected findings",
            "description": "Report filtering workflow failure",
            "url": "https://github.com/pproenca/codenuke/issues/22",
