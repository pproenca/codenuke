---
name: codenuke-issue-digest
description: Run a GitHub issue digest for pproenca/codenuke by feature-area labels, all areas, and configurable time windows. Use when asked to summarize recent codenuke bug reports or enhancement requests, especially for areas such as cli, mapper, provider, workflow, docs, packaging, or similar labels.
---

# Codenuke Issue Digest

## Objective

Produce a headline-first, insight-oriented digest of `pproenca/codenuke` issues for the requested feature-area labels over the previous 24 hours by default. Honor a different duration when the user asks for one, for example "past week" or "48 hours". Default to a summary-only response; include details only when requested.
...
- Feature-area labels, for example `cli mapper provider workflow docs packaging`
- `all areas` / `all labels` to scan all current feature labels
- Optional repo override, default `pproenca/codenuke`
- Optional time window, default previous 24 hours; examples: `48h`, `7d`, `1w`, `past week`

## Workflow

1. Run the collector from a current codenuke repo checkout:

```bash
python3 .codex/skills/codenuke-issue-digest/scripts/collect_issue_digest.py --labels cli mapper --window-hours 24
```
...
🔥🔥 Review resumes duplicate findings [1](https://github.com/pproenca/codenuke/issues/123)
🔥 Provider schema rejects valid mock output [2](https://github.com/pproenca/codenuke/issues/456)
...
7. Use inline numbered references where a row or bullet points to issues, for example `Mapper regressions [1](https://github.com/pproenca/codenuke/issues/123), [2](https://github.com/pproenca/codenuke/issues/456)`. Do not add a separate footnotes section.
...
```text
Use $codenuke-issue-digest to run the codenuke issue digest for labels cli and mapper over the previous 24 hours.
```

```text
Use $codenuke-issue-digest to run the codenuke issue digest for all areas over the past week.
```
...
```bash
python3 .codex/skills/codenuke-issue-digest/scripts/collect_issue_digest.py --all-labels --window "past week" --limit-issues 10
```

Run the focused script tests:

```bash
pytest .codex/skills/codenuke-issue-digest/scripts/test_collect_issue_digest.py
```
