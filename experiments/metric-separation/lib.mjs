...
function coupling(files) {
  ...
  const modules = Object.keys(files).filter((n) => !isTest(n)).map(base);
...
export function typeErrors(files) {
  ...
  try {
    ...
    host.getSourceFile = (fileName, lang, onErr) => {
      ...
      if (local) return ts.createSourceFile(fileName, map.get(local), lang, true, scriptKind(local));
      return og(fileName, lang, onErr);
    };
    const ogExists = host.fileExists.bind(host);
    host.fileExists = (f) => [...map.keys()].some((k) => f === k || f.endsWith("/" + k)) || ogExists(f);
...
const H_PREAMBLE =
  "const h=(t,p,...c)=>({__node:true,t,p:p||{},c:c.flat(Infinity)});\n";

function rewriteSpecifiers(js) {
  return js.replace(
    /((?:import|export)[^;\n]*?\bfrom\s*['"])(\.[^'"]+)(['"])/g,
    (m, a, spec, z) => (/\.[tj]sx?$|\.mjs$/.test(spec) ? m : `${a}${spec}.mjs${z}`),
  );
}
