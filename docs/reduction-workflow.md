...
| 2026-05-23 | Reuse doctor helpers         | `doctor`; subprocess checks and isolated readiness worktree cleanup                                   | Confirmed doctor readiness/cleanup tests plus shell/worktree helper tests                        | `pnpm typecheck`; focused Vitest; `pnpm lint`; `pnpm test`; `pnpm build`; `pnpm eval`; `pnpm pack:smoke`; `pnpm format:check`; `git diff --check`                                                           |       -15 |       -62 |               -2 |        -9 | done   | After: LOC 3229, AST 20989, complexity 708, dupMass 161. Risk: low; doctor now reuses shared shell/worktree helpers while preserving readiness output.          |
| 2026-05-23 | Extract finite-number guard  | `artifacts`, `validate-proxy`; finite numeric validation for fail-closed artifact/candidate parsing   | Added focused guard tests; confirmed artifact/value-proxy malformed-input tests                  | `pnpm typecheck`; focused Vitest; `pnpm lint`; `pnpm test`; `pnpm build`; `pnpm eval`; `pnpm pack:smoke`; `pnpm format:check`; `git diff --check`                                                           |        -5 |        -2 |                0 |         0 | done   | After: LOC 3224, AST 20987, complexity 708, dupMass 161. Risk: low; same finite-number predicate now shared from `guards`.                                      |
| 2026-05-23 | Extract loop fixture helpers | Deterministic eval and package smoke fixture generation and file writes                               | Confirmed `pnpm eval` and `pnpm pack:smoke` exercise fixture source and file creation            | `pnpm eval`; `pnpm pack:smoke`; `pnpm typecheck`; `pnpm lint`; `pnpm test`; `pnpm build`; `pnpm format:check`; `git diff --check`                                                                           |        -7 |       -16 |               +1 |       -16 | done   | After: LOC 3217, AST 20971, complexity 709, dupMass 145. Risk: low; generated TypeScript fixture text and fixture file-write behavior are unchanged.            |
| 2026-05-23 | Reuse source-file predicate  | `config`; source directory detection and exported source-file filtering                               | Confirmed `loop/config.test.mjs` source detection coverage                                       | `pnpm test loop/config.test.mjs`; `pnpm typecheck`; `pnpm lint`; `pnpm test`; `pnpm build`; `pnpm eval`; `pnpm pack:smoke`; `pnpm format:check`; `git diff --check`                                         |        -2 |       -29 |               -2 |         0 | done   | After: LOC 3215, AST 20942, complexity 707, dupMass 145. Risk: low; internal source detection now uses the same predicate exported to scorer/fence/calibration. |
| 2026-05-23 | Extract source-dir predicate | `run`, `changecost`; reduce/implementer dirty-path safety filters                                     | Confirmed `loop/autoloop.test.mjs` and `loop/changecost.test.mjs` safety-surface coverage        | `pnpm test loop/autoloop.test.mjs loop/changecost.test.mjs`; `pnpm typecheck`; `pnpm lint`; `pnpm test`; `pnpm build`; `pnpm eval`; `pnpm pack:smoke`; `pnpm format:check`; `git diff --check`              |        -2 |       -28 |               -2 |         0 | done   | After: LOC 3213, AST 20914, complexity 705, dupMass 145. Risk: low; callers still keep their reduce/implementer-specific file-type policies local.              |
