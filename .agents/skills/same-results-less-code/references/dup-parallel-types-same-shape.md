...
When `User`, `Customer`, and `Contact` all carry `{id, name, email, phone}` and the difference is only "where in the system they live," you have one shape with three labels, not three things. The duplication is invisible to linters because the _names_ are different, but every consumer ends up writing the same logic three times and every change happens in triplicate. Either unify them, or — when they're genuinely distinct — make the distinction the _only_ thing that differs.
...
- The types come from distinct external systems with their own naming conventions you don't control (a DB schema, a SOAP API). Keep them separate at the boundary, but map to one internal type _once_ — not at every consumer.
- The types share fields _today_ but are expected to diverge along well-known axes. Premature unification can hurt; lock them in only when both directions are stable.
