...
Look at the _body line by line_. If every difference between the two functions is:

- A literal value (string, number, currency code), or
- A different table/collection name, or
- A different audit category

...then the function is one function. If the differences include _different control flow, different error handling, different validation rules_, then they may genuinely be two functions — and you should split that distinction into shared and divergent parts, not collapse them.
