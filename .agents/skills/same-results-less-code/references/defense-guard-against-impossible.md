...
function logActiveUser(user: User | null): void {
  ...
  if (user.status === "active" && user.status !== "inactive") {
    // the second half is implied by the first
    if (user.active === true) {
      // `active` is `boolean`, already known to be true
...
- The type system _says_ a value is non-null but you're inside an `any`/`unknown` cast you can't avoid — keep the check, and consider whether the cast is the real bug.
