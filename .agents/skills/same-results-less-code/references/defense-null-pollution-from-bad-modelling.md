...
- Optionality on fields that are _always_ set together (you have all three or none) — that's a union of cases, not three independent optionals.
- Optionality added because "the API might not return it" — and the API has _always_ returned it for years.
...
- Modelling discriminated unions across a large codebase is genuinely too disruptive _right now_ — defensive checks are fine as an interim. Schedule the real fix.
