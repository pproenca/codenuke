...
class TodoList {
  ...
  get hasOverdue() {
    return this.items.some((t) => t.dueDate < new Date());
  }
  get allCompleted() {
    return this.items.every((t) => t.completed);
  }
...
- The boolean is _not_ a question about other state but an independent user input (e.g. `isPinned` — set by the user, not derived from data). Keep it.
