...
But machine-global bun availability does not mean the *repo* uses bun. On `../codecharter` (2026-05-22, bun installed) detection returned `bun test`, which fails — codecharter's tests run via `pnpm test` → `tsx test-support/run-tests.mts`. `doctor` reported `test: not-ready (bun test)` until overridden with `CN_TEST="pnpm test"` (now pinned in codecharter's `codenuke.loop.json`).
